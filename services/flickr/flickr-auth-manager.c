...
static void
flickr_auth_mgr_on_token_checked (RestProxyCall * call,
                                  const GError * error,
				  GObject * auth_manager,
				  gpointer callback_data);
  ...
  if (g_strcmp0 (url_format, FLICKR_REST_API_URL) == 0)
  ...
  else
    {
      ...
      g_object_get (G_OBJECT (proxy),
		    ...
		    "token", &token, "token-secret", &token_secret,
                    NULL);
				   ...
				   gpointer user_data, GError ** error)
{
  ...
  proxy = oauth_proxy_new_with_token (consumer_key,
				      ...
				      token,
				      token_secret,
				      FLICKR_REST_API_URL, NO_BINDING);
...
flickr_auth_mgr_get_access_token_async (FlickrAuthManager * self,
					...
					access_callback, gpointer user_data,
					GError ** error)
{
